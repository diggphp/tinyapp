...
``` bash
composer require diggphp/tinyapp
```

## 用例

``` php
$app = new \DiggPHP\TinyApp\TinyApp();
$app->get('[/]', function(){
    return 'hello world~';
});
$app->run();
...
// 路由分组
$app->addGroup('/news', function (\DiggPHP\Router\Collector $collector) {
    $collector->get('/lists', 'somefuncion');
    $collector->get('/detail/{id:\d+}', 'somefuncion2');
}, ['中间件绑定'], ['参数绑定']);

// 多级路由分组
$app->addGroup('/article', function (\DiggPHP\Router\Collector $collector) {
    $collector->get('/lists', 'somefuncion');
    $collector->get('/detail/{id:\d+}', 'somefuncion2', ['中间件绑定'], ['参数绑定']);
    $collector->addGroup('/comment', function(\DiggPHP\Router\Collector $collector){
...
class SomeClass{
    public function action(
        \DiggPHP\Request\Request $request
    ){
